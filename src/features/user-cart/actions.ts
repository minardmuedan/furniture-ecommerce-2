...
export const addToCartAction = createAuthedServerAction(addToCartSchema)
  ...
  .handle(async (session, { productId, quantity }, throwFieldError) => {
    const id = generateSecureRandomString()

    const product = await getCachedProduct(productId)
    if (!product) throw throwFieldError({ productId: 'Product dont exist, Please try again' })
...
export const updateCartQtyAction = createAuthedServerAction(updateCartQtySchema)
  ...
  .handle(async (session, { cartId, quantity }) => {
    ...
    if (!cartData || cartData.userId !== session.user.id) throw new CustomError('not_found', 'Product cart data not found!')
